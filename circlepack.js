...
function genericCirclePackLayout(assign, graph, options) {
    ...
    graph.forEachNode((key, attributes) => {
        var nodeAttr = [];
        var newCircleWrap = new CircleWrap();
        var parentContainer = container;
        newCircleWrap.id = key;
        newCircleWrap.r = attributes['size'] || 1;

        hierarchyAttributes.forEach(v => nodeAttr.push(attributes[v]));
        nodeAttr.forEach(v => {
    ...
    packHierarchyAndShift(container);
    ...
    for (i = 0; i < l; i++) {
        ...
        x = center + scale * posMap.get(node).x;
        y = center + scale * posMap.get(node).y;
...
function setNode(/*Graph*/ graph, /*CircleWrap*/ parentCircle, /*Map*/posMap) {
    parentCircle.children.forEach((circle, key) => {
        if (circle.hasChildren()) {
            setNode(graph, circle, posMap);
        } else {
            posMap.set(circle.id, {x: circle.x, y: circle.y});
        }
    })
...
function encloseBasis1(/*CircleWrap*/a) {
    return CircleWrap.fromValues(a.x, a.y, a.r);
}
function encloseBasis2(/*CircleWrap*/ a, /*CircleWrap*/ b) {
    ...
    return CircleWrap.fromValues((x1 + x2 + x21 / l * r21) / 2,
        (y1 + y2 + y21 / l * r21) / 2,
        (l + r1 + r2) / 2);
}
...
function encloseBasis3(/*CircleWrap*/a, /*CircleWrap*/b, /*CircleWrap*/c) {
    ...
    return CircleWrap.fromValues(x1 + xa + xb * r, y1 + ya + yb * r, r);
...
function packEnclose(/*Array<CircleWrap>*/ circles) {
    ...
    a = CircleWrap.fromCircleWrap(a);
    b = CircleWrap.fromCircleWrap(b);
    c = CircleWrap.fromCircleWrap(c);
    a.next = c.previous = b;
    b.next = a.previous = c;
    c.next = b.previous = a;
    ...
    for (i = 3; i < n; ++i) {
        ...
        do {
            if (sj <= sk) {
                ...
                sk += k.wrappedCircle.r, k = k.previous;
            }
        } while (j !== k.next);
...
function packHierarchy(/*CircleWrap*/ parentCircle) {
    ...
    if (parentCircle.hasChildren()) {
        ...
        r = packEnclose(Array.from(parentCircle.children.values()));
    }
    return r;
}

function packHierarchyAndShift(/*CircleWrap*/ parentCircle) {
    var r = packHierarchy(parentCircle);
    parentCircle.children.forEach((circle, key) => {
